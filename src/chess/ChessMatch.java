...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (testCheck(currentPlayer)) {
			...
			throw new ChessException("Forbidden move: imminent check position.");
		}
	...
	private void validateTargetPosition(Position source, Position target) {
		if (!board.piece(source).possibleMove(target)) {
			throw new ChessException("Forbidden move.");
		}
	}
	...
	private ChessPiece king(Color color) {
		...
		throw new IllegalStateException("Error: no " + color + " King found on the board!");
	}
		
	private boolean testCheck(Color color) {
		//return testIfPositionUnderThreat(color, king(color).getChessPosition().toPosition());
	...
	public boolean isPositionUnderThreat(Color color, Position position) {
		List<Piece> opponentPieces = piecesOnTheBoard.stream().
				filter(x -> ((ChessPiece)x).getColor() == opponent(color)).collect(Collectors.toList());
		for (Piece p : opponentPieces) {
			boolean[][] mat = p.possibleMoves();
